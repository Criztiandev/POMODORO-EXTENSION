...
export class TodoManager {
  ...
  private loadState(): void {
    try {
      ...
      if (savedState && savedState.tasks && Array.isArray(savedState.tasks)) {
        ...
        console.log('TodoManager: Loaded and parsed state', this.state);
        
        this.cleanupCompletedTasks();
        this.ensureTaskLimit();
        
        console.log('TodoManager: State after cleanup', this.state);
      } else {
        console.log('TodoManager: No saved state found, initializing empty');
  ...
  private async saveState(): Promise<void> {
    try {
      console.log('TodoManager: Attempting to save state', this.state);
      
      // Serialize state with Date objects converted to ISO strings
      ...
      console.log('TodoManager: Serializable state', serializableState);
      
      const config = vscode.workspace.getConfiguration('pomodoroTimer');
      
      // Try workspace first, fallback to global
      try {
        await config.update(TodoManager.TODO_KEY, serializableState, vscode.ConfigurationTarget.Workspace);
        console.log('TodoManager: Saved to workspace configuration');
      } catch (workspaceError) {
        console.warn('TodoManager: Workspace save failed, trying global', workspaceError);
        await config.update(TodoManager.TODO_KEY, serializableState, vscode.ConfigurationTarget.Global);
        console.log('TodoManager: Saved to global configuration');
      }
    } catch (error) {
      console.error('TodoManager: Failed to save state', error);
  ...
  private cleanupCompletedTasks(): void {
    ...
    this.state.tasks = this.state.tasks.filter(task => {
      ...
      if (task.completedAt) {
        const daysSinceCompleted = (now.getTime() - task.completedAt.getTime()) / (1000 * 60 * 60 * 24);
  ...
  private sortTasks(): void {
    this.state.tasks.sort((a, b) => {
      ...
      if (a.id === this.state.currentTaskId) {return -1;}
      if (b.id === this.state.currentTaskId) {return 1;}
  ...
  getCurrentTask(): Todo | undefined {
    if (!this.state.currentTaskId) {return undefined;}
